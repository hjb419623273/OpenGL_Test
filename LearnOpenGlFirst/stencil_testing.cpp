//模板测试 Stencil Test
/*
片段着色器处理完一个片段后 模板测试开始执行 和深度测试一样 也可能会丢弃片段 被保留的片段会进入深度测试 
可能会丢弃更多片段

模板测试是根据另一个缓冲来进行的 叫做模板缓冲 Stencil Buffer 

一个模板缓存中 灭个模板值是8位 每个像素/片段一共能有256种不同的模板值

模板缓冲首先会被清除为0 之后在模板缓冲中使用1填充一个空心矩阵 场景中的片段将会只在片段的模板值为1的时候会被
渲染 其它的都会被丢弃

模板缓冲操作允许我们在渲染片段时将模板缓冲设定为一个特定的值。 通过在渲染时修改模板缓冲的内容,我们写入了模板
缓冲。 在同一个渲染迭代中 我们可以读取这些值，来决定丢弃还是保留某个片段。
启用模板缓冲的写入
渲染物体,更新模板缓冲的内容
禁用模板缓冲的写入
渲染物体,这次根据模板缓冲的内容丢弃特地片段

glStencilMask 允许我们设置一个掩码bitmask，它会与将要写入缓冲的模板值进行与AND运算 默认情况下位掩码所有位都为1

glStencilFunc  glStencilOp
glStencilFunc(GLenum func, GLint ref, GLuint mask)
func 设置模板测试函数 将会将应用到易存储的模板值上和glStencilFunc函数的ref值上
ref 设置了模板测试的参考值
mask 设置掩码
...
1.在绘制物体前，将模板函数设置为GL_ALWAYS, 每当物体的片段被渲染时，将模板缓冲更新为1
2.渲染物体
3.禁用模板写入以及深度测试
4.将每个物体缩放一点
