/*
	...
	作用：渲染你的场景到不同的帧缓冲能够让我们在场景中加入类似镜子的东西，或者做出很酷的后期处理效果
	
	颜色缓冲 深度缓冲 模板缓冲 这些缓冲结合起来叫做帧缓冲
	glGenFramebuffers的函数来创建一个帧缓冲对象(Framebuffer Object, FBO)
	...
	3所有附件都必须是完整的
	4每个缓冲都应该有相同的样本数
	所以我们要为帧缓冲创建一些附件并将附件附加到帧缓冲上。
	由于我们的帧缓冲不是默认帧缓冲，渲染指令将不会对窗口的视觉输出有任何影响。出于这个原因，渲染到一个不同的帧缓冲被叫做离屏渲染(Off-screen Renderin)

纹理附件
	当把一个纹理附加到帧缓冲的时候 所有的渲染指令都将会写入到这个纹理中。就像它是一个普通的颜色/深度或者模板缓冲一样。使用纹理的优点是，所有
	渲染操作的结果将会被储存在一个纹理图像中，我们之后可以在着色器中方便使用它。
	我们将维度设置为了屏幕大小（尽管这不是必须的），并且我们给纹理的data参数传递了NULL。对于这个纹理，我们仅仅分配了内存而没有填充它。填充这个纹理将会在我们渲染到帧缓冲之后来进
	行。
...
渲染缓冲对象附件
	渲染缓冲对象 renderbuffer object 是在纹理之后引入到opengl中，作为一个可用的帧缓冲附件类型的，所以
	在过去纹理是唯一可用的附件。和纹理图像一样，渲染缓冲对象是一个真正的缓冲，即一系列的字节、整数、像素等。渲染缓冲对象附加的好处是，
